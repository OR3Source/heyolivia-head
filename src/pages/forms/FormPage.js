...
const FormPage = () => {
    ...
    const hiddenFormRef = useRef(null); // Added for Netlify hidden form
    ...
    useEffect(() => {
        ...
        const t = p.get("type");
        if (t) setFormData(f => ({ ...f, helpTopic: t }));
    }, []);

    useEffect(() => {
        if (phase1Complete && phase2Ref.current) {
            const t = phase2Ref.current.getBoundingClientRect().top + window.scrollY - 180;
            window.scrollTo({ top: t, behavior: "smooth" });
        }
    }, [phase1Complete]);

    useEffect(() => {
        if (phase2Locked && phase3Ref.current) {
            const t = phase3Ref.current.getBoundingClientRect().top + window.scrollY - 180;
            window.scrollTo({ top: t, behavior: "smooth" });
        }
    }, [phase2Locked]);
    ...
    useEffect(() => {
        ...
        const e = document.querySelector('.event-submission-page');
        if (!e) return;

        const g = () => Array.from(e.querySelectorAll('.user-info-section'));
        let r = null;
        ...
        const c = () => {
            ...
            r = requestAnimationFrame(() => {
                const s = g();
                const t = s.reduce((a, b) => a + Math.ceil(b.getBoundingClientRect().height), 0);
                const w = window.innerWidth;
                let vh = w <= 360 ? 85 : w >= 361 && w <= 399 ? 75 : w >= 400 && w <= 768 ? 85 : w >= 769 && w <= 1024 ? 95 : 100;
        ...
        m.observe(e, { childList: true, subtree: true, attributes: true, attributeFilter: ['class', 'style'] });

        const f = () => c();
        window.addEventListener('resize', f);

        let i = null;
        if (typeof ResizeObserver === 'undefined') i = setInterval(c, 700);
    ...
    const FileUpload = ({ files, setFiles }) => {
        const { getRootProps, getInputProps, isDragActive } = useDropzone({
            onDrop: (a, r) => {
                ...
                if (a.length > 4) {
                    nA = a.slice(0, 4);
                    nR = a.slice(4).map(f => ({ file: f, reason: "too-many" }));
                } else {
                    nA = a;
                    nR = r.map(x => ({ file: x.file, reason: "invalid-type" }));
                }
                setFiles({ accepted: nA, rejected: nR });
                setFormData(f => ({ ...f, file: nA.length > 0 ? nA[0] : null }));
                if (hiddenFileInputRef.current) hiddenFileInputRef.current.files = (t => {
                    const dt = new DataTransfer();
                    t.forEach(x => dt.items.add(x));
                    return dt.files;
                })(nA);
            },
        ...
        return <div className="dropzone-wrapper">
            ...
            {files.accepted.length > 0 && <div className="uploaded-files">
                <div className="file-status">
                    <img src={NavPlus} alt="Accepted" className="file-status-icon" />
                    <div className="file-names">{files.accepted.map((f, i) => <div key={i} className="file-name">{f.name}</div>)}</div>
                </div>
            </div>}
            {files.rejected.length > 0 && <div className="rejected-files">
                <div className="file-status">
                    <img src={NavMinus} alt="Rejected" className="file-status-icon" />
                    <div className="file-names">{files.rejected.map(({ file }, i) => <div key={i} className="file-name">{file.name}</div>)}</div>
                </div>
                <div className="file-error-message">
                    {files.rejected.some(f => f.reason === "too-many") ? <div className="error-msg">TOO MANY FILES UPLOADED</div> :
                        files.rejected.some(f => f.reason === "invalid-type") ? <div className="file-error-message">INVALID FILE SUBMISSION</div> :
    ...
    const validatePhase1 = () => {
        ...
        if (identityMethod === "name") {
            ...
            if (!formData.twitter.trim()) e.twitter = "Username required";
        }
        return e;
    };
    ...
    return <div className="event-submission-page">
        <Helmet><title>heylivies | form submission</title></Helmet>
                    ...
                    <div className="form-fields">
                        <div className="name-row">
                            <div className="field-group">
                                ...
                                <input type="text" id="firstName" name="firstName" value={formData.firstName} onChange={handleInputChange} className={errors.firstName ? "error" : ""} />
                                {errors.firstName && <span className="error-msg">{errors.firstName}</span>}
                            </div>
                            <div className="field-group">
                                <label htmlFor="lastName">Last Name</label>
                                <input type="text" id="lastName" name="lastName" value={formData.lastName} onChange={handleInputChange} className={errors.lastName ? "error" : ""} />
        ...
        {/* PHASE 2 */}
        {phase1Complete && <div ref={phase2Ref} className={`user-info-section ${phase2Locked ? "locked collapsed" : ""}`}>
            ...
            {phase2Locked && <div className="collapsed-summary">{formData.helpTopic}
                <button className="edit-btn" onClick={() => setPhase2Locked(false)}><img src={editImage} alt="edit" /></button>
            </div>}
        </div>}

        {/* PHASE 3 */}
        {phase2Locked && <div ref={phase3Ref} className={`user-info-section ${phase3Locked ? "locked collapsed" : ""}`}>
            <h2 className="section-heading">PHASE 3: DETAILS / FILE UPLOAD</h2>
            <div className="bookmark-icon"></div>
            {!phase3Locked && <div className="form-fields">
                <ReactQuill theme="snow" value={formData.details} onChange={v => setFormData(f => ({ ...f, details: v }))} modules={modules} className={errors.details ? "error quill-editor" : "quill-editor"} placeholder="Enter details here..." />
                {errors.details && <span className="error-msg2">{errors.details}</span>}
                {["media", "event", "bug"].includes(formData.helpTopic) && <div className="field-group">
            ...
            {phase3Locked && <div className="collapsed-summary">Details submitted
                <button className="edit-btn" onClick={() => setPhase3Locked(false)}><img src={editImage} alt="edit" /></button>
            </div>}
        </div>}

        {/* PHASE 4 */}
        {phase3Complete && !phase4Locked && <div className="user-info-section">
            <h2 className="section-heading">PHASE 4: CONFIRMATION</h2>
            <div className="form-fields">
                <label className="checkbox-label"><input type="checkbox" checked={phase4Checked}
                                                         onChange={e => setPhase4Checked(e.target.checked)}/> I confirm
                    that all information is entered correctly.</label>
            </div>
            <button
                type="button"
                disabled={!phase4Checked}
                onClick={(e) => {
                    e.preventDefault();
                    ...
                    // If Twitter method is used, populate name/email fields
                    const dataToSubmit = {...formData};
                    if (identityMethod === "twitter") {
                        const username = formData.twitter || "user";
                        dataToSubmit.firstName = username;
                        dataToSubmit.lastName = username;
                        dataToSubmit.email = `${username}@twitter.com`;
                    }

                    Object.keys(dataToSubmit).forEach(key => {
                        const input = formEl.querySelector(`[name="${key}"]`);
                        if (input) input.value = dataToSubmit[key] || "";
                    });
                    formEl.submit();
                }}
            >
