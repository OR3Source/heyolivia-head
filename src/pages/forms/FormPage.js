...
const FormPage = () => {
    ...
    // Scroll to phase 2 on unlock
    useEffect(() => {
        if (phase1Complete && phase2Ref.current) {
            const t =
                phase2Ref.current.getBoundingClientRect().top +
                window.scrollY -
                180;
    ...
    useEffect(() => {
        if (phase2Locked && phase3Ref.current) {
            const t =
                phase3Ref.current.getBoundingClientRect().top +
                window.scrollY -
                180;
            window.scrollTo({ top: t, behavior: "smooth" });
        }
    ...
    useEffect(() => {
        ...
        const c = () => {
            ...
            r = requestAnimationFrame(() => {
                const s = g();
                const t = s.reduce(
                    (a, b) => a + Math.ceil(b.getBoundingClientRect().height),
                    0
                );
        ...
        const { getRootProps, getInputProps, isDragActive } = useDropzone({
            onDrop: (acceptedFiles, rejectedFiles) => {
                ...
                if (acceptedFiles.length > 4) {
                    validFiles = acceptedFiles.slice(0, 4);
                    rejected = acceptedFiles
                        .slice(4)
                        .map((f) => ({ file: f, reason: "too-many" }));
                } else {
                    validFiles = acceptedFiles;
                    rejected = rejectedFiles.map((x) => ({
                        file: x.file,
                        reason: "invalid-type",
                    }));
                }

                setFiles({ accepted: validFiles, rejected });
    ...
    const validatePhase2 = () => {
        const e = {};
        if (!formData.helpTopic.trim()) e.helpTopic = "Select an option";
        return e;
    };
    ...
    const validatePhase3 = () => {
        ...
        if (!formData.details.trim())
            e.details = "Please provide details";
        if (
            ["media", "event", "bug"].includes(formData.helpTopic) &&
            !formData.file
        )
            e.file = "File required for this category";
        return e;
    };

    // Phase Handlers
    const handlePhase1Done = () => {
        const e = validatePhase1();
        setErrors(e);
        if (!Object.keys(e).length) {
            setPhase1Complete(true);
            setPhase1Locked(true);
            if (progress < 33) setProgress(33);
        }
    };

    const handlePhase2Done = () => {
        const e = validatePhase2();
        setErrors((f) => ({ ...f, ...e }));
        if (phase1Complete && !Object.keys(e).length) {
    ...
    const handlePhase3Submit = () => {
        const e = validatePhase3();
        setErrors((f) => ({ ...f, ...e }));
        if (!Object.keys(e).length) {
            setPhase3Locked(true);
            setPhase3Complete(true);
            setPhase4Locked(false);
    ...
    return (
        <div className="event-submission-page">
            <Helmet>
                <title>heylivies | form submission</title>
            </Helmet>

            <div className="top-form">
                <h1>HL FORM SUBMISSION</h1>
                <p className="subtitle">
                    "I feel your compliments like bullets on skin." - Lacy
                </p>
                        ...
                        <div className="identity-choice">
                            <label className="radio-label">
                                <input
                                    ...
                                    onChange={(e) =>
                                        setIdentityMethod(e.target.value)
                                    }
                                />
                ...
                >
                    ...
                    {!phase3Locked && (
                        <div className="form-fields">
                            ...
                            />
                            {errors.details && (
                                <span className="error-msg2">
                                    {errors.details}
                                </span>
                            )}
                            {["media", "event", "bug"].includes(
                                formData.helpTopic
                            ) && (
                                <div className="field-group">
                    ...
                    <div className="form-fields">
                        <label className="checkbox-label">
                            <input
                                ...
                                onChange={(e) =>
                                    setPhase4Checked(e.target.checked)
                                }
                            />{" "}
                            I confirm that all information is entered correctly.
                        </label>
                    </div>
                    <button
                        type="button"
